...
3) il file Python "strategy_environments_variables", il quale contiene le variabili di ambiente che descrivono i parametri operativi principali della strategia. La variabile "SYMBOL" descrive il TAG dell'asset sul quale intendiamo investire. "LOT" è un numero che descrive l'importo di ogni investimento. Ala variabile "MAGIC" invece bisogna fornire la "targa numerica" arbitraria che ci permetterà di distinguere le operazioni eseguite da diverse strategie che operano in simultanea sul emdesimo conto, ed ovviamente ci aiuta ad evitare che le diverse strategie vadano in conflitto tra loro.
